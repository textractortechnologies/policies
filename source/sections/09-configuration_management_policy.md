...
Textractor standardizes and automates configuration management through the use of Healthcare Blocks Dokku Platform-as-a-Service as well as documentation of all changes to production systems and networks.  Healthcare Blocks Dokku Platform-as-a-Service automatically configure all Textractor systems according to established and tested policies, and are used as part of our Disaster Recovery plan and process.
...
3. All changes to production systems, network devices, and firewalls are approved by the Textractor CTO before they are implemented to assure they comply with business and security requirements.
...
6. Tooling to generate an up-to-date inventory of systems, including corresponding architecture diagrams for related products and services, is hosted on GitLab.
   ...
   * These scripts are used to generate the diagrams and asset lists required by the Risk Assessment phase of Textractor's Risk Management procedures ([§4.3.1](#4.3-risk-management-procedures)).
...
10. Textractor utilizes development and staging environments that mirror production to assure proper function.
11. Textractor also deploys environments locally using Docker to assure functionality before moving to staging or production.
...
1. Before provisioning any systems, ops team members must file a request in the Textractor Quality Management System.
...
3. Critical infrastructure services such as logging, monitoring, LDAP servers, or Windows Domain Controllers must be configured with appropriate Salt states.
   * These Salt states have been approved by the CTO, or an authorized delegate of the CTO, to be in accordance with all Textractor policies, including setting appropriate:
...
1. Textractor uses automated tooling to ensure systems are up-to-date with the latest security patches.
...
5. If the feature or defect interacts with ePHI, or controls access to data potentially containing ePHI, the code changes must be reviewed by two members of Textractor's Blue Team (software security team) before the feature is marked as complete.
   * The Blue Team members will provide a security analysis of features to ensure they satisfy Textractor's compliance and security commitments.
   ...
   * Blue Team members are required to undergo annual training on identifying the most common software vulnerabilities and will receive ongoing training on Textractor's compliance and security requirements.
